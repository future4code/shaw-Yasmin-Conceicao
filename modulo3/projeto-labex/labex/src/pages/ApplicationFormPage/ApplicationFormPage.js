//para o usuario se candidatar á viagens, pagina que vai ter o formulario de inscrição 
...
import { ContainerInscricao} from "./styled"
...
export default function ApplicationFormPage(){
  ...
  axios.post(`${BASE_URL}/${chooseTrip}/apply`, body)
  .then((res)=>{
    applyToTrip(res.data.trips)
  ...
  const onChangeIdade = (ev) =>{
    setIdade(ev.target.value)
  }

  const onChangePais = (ev) =>{
  ...
  const onChangeProfissao = (ev) =>{
    setProfissao(ev.target.value)
  }

  const onChangeChooseTrip = (ev) =>{
    setChooseTrip(ev.target.value)
  }
  ...
  const goBack = (navigate) =>{
    ...
    return(
      <div>
        <ContainerInscricao>
        <Typography variant="h1" align={'center'} gutterBottom>Inscrição</Typography>

        <Input placeholder="Nome" onChange={onChangeName} value={nome} color="secondary"/>

        <Input placeholder="Idade" type='number' onChange={onChangeIdade} value={idade} color="secondary"/>

        <Input placeholder="Texto de candidatura"  onChange={onChangeTexto} value={texto} color="secondary" />

        <Input placeholder="Profissão" onChange={onChangeProfissao} value={profissao} color="secondary"/>
     
        <FormControl variant="standard" sx={{ m: -0.10, minWidth: 120 }}>
        <InputLabel id="demo-simple-select-standard-label">Escolha uma viagem</InputLabel>
        <Select
        labelId="demo-simple-select-label"
        id="demo-simple-select"
        value={chooseTrip}
        onChange={onChangeChooseTrip}
        > 
        ...
        id="demo-simple-select" value={pais} onChange={onChangePais}>
        <MenuItem value={''}>Escolha um País</MenuItem>
      ...
      {/* <Button onClick={() => goBack(navigate)} variant={'contained'} color={'primary'}>enviar</Button> */}
     
      {/* <Button variant={'outlined'} color={'secondary'}>voltar</Button> */}

      <Button variant={'contained'} color={'primary'} onClick={applyToTrip}>enviar</Button>
