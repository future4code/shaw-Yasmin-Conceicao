import React, { useEffect, useState } from "react";
import { BoxImgLeft, BoxImgRight, BoxTitle, BoxTitleFooter, ContainerLeft, ContainerMain, FifthCircle, FirstCircle, FourthCircle, ImgStyled, Loading, NumberCircle, NumberFooter, SecondCircle, SeventhCircle, SixthCircle, StyledCirculo, ThirdCircle } from "./styled";
import Sidebar5 from "../../Img/Sidebar5.png"
import Path from "../../Img/Path.png"
import Path2 from "../../Img/Path2.png"
import concurso from "../../Img/concurso.png"
import timemania from "../../Img/timemania.png"
import circulo from "../../Img/circulo.png"
import MenuItem from '@mui/material/MenuItem';
import Select from '@mui/material/Select';
import { useNavigate } from "react-router-dom";
import { goToDiaDeSorte, goToLotofacil, goToLotomania, goToMegaSena, goToQuina, goToTimemania } from "../../Routes/Coordinator";
import axios from "axios";
import { BASE_URL } from "../../Components/BaseUrl";
import moment from "moment";
import CircularProgress from '@mui/material/CircularProgress';


export default function Timemania () {

        const navigate = useNavigate()
        const [numberContests , setNumberContests] = useState([])
        const [lotteries, setLotteries] = useState([])
        const [date, setDate] = useState("")
    
        const getLotteries = async () =>{
           await axios.get(`${BASE_URL}/loterias`)
            .then((res)=> {
              setLotteries(res.data)
            })
            .catch((err)=>{
                alert(err.response.data.message)
            })
          }

        const getContests = () => {
            axios.get(`${BASE_URL}/loterias-concursos`)
            .then((res) => {
                setNumberContests(res.data)
            })
            .catch((err) => {
                alert(err.response.data.message)
            })
        }

        const getDate = async () => {

            if(numberContests.length > 0 ){
    
            const lotery = lotteries.filter((lottery) => {
                return(
                    lottery.nome === "timemania"
                )
            })
    
            const contest = numberContests.filter((numberContest)=>{
                return(
                    numberContest.loteriaId === lotery[0].id
                )
            })
          
            await axios.get(`${BASE_URL}/concursos/${contest[0].concursoId}`)
            .then((res) => {
                setDate(res.data)
                console.log(res.data)
            })
            .catch((err) => {
                alert(err.response.data.message)
            })
    
            }
        }
    
        const mapContests = numberContests.map((contest) => {
            return(
                contest.concursoId
            )
        })
     
        
        useEffect(() => {
            getContests()
            getLotteries()   
          
        },[])
    
    
        useEffect(() => {
          getDate()  
        },[numberContests])
    
    
        const pushArray = lotteries.map((lottery)=> {
            return(
               <MenuItem value={lottery.id}>{lottery.nome}</MenuItem>
            )
        })

    return(
        
        <ContainerMain>

            <ImgStyled src={Sidebar5}/>

          <ContainerLeft>
           <Select sx={{width:"215.91px", height:"45.2px", backgroundColor: "white", borderRadius: "10px"}}
              displayEmpty
              inputProps={{ 'aria-label': 'Without label' }}
              >
              <MenuItem onClick={() => goToMegaSena(navigate)}>
                  <em>MEGA-SENA</em>
              </MenuItem>
              <MenuItem onClick={() => goToQuina(navigate)}>QUINA</MenuItem>
              <MenuItem onClick={() => goToLotofacil(navigate)}>LOTOFÁCIL</MenuItem>
              <MenuItem onClick={() => goToLotomania(navigate)}>LOTOMANIA</MenuItem>
              <MenuItem onClick={() => goToTimemania(navigate)}>TIMEMANIA</MenuItem>
              <MenuItem onClick={() => goToDiaDeSorte(navigate)}>DIA DE SORTE</MenuItem>
            </Select>

              <BoxTitle>
                  <img src={timemania}/>
              </BoxTitle>

              <BoxImgRight>
                  <img src={Path2}/>
              </BoxImgRight>

              <BoxImgLeft>
                  <img src={Path}/>
              </BoxImgLeft>

              <BoxTitleFooter>
                  <img src={concurso}/>
                  <NumberFooter>{mapContests[4]} - {moment(date.data).format("DD/MM/YYYY")}</NumberFooter>
              </BoxTitleFooter>

          </ContainerLeft>  

          
          {date.numeros?
          
          <>

          <FirstCircle>
              <StyledCirculo src={circulo}/>   
              <NumberCircle>{date?.numeros[0]}</NumberCircle>
          </FirstCircle>

          <SecondCircle>
              <StyledCirculo src={circulo}/>
              <NumberCircle>{date?.numeros[1]}</NumberCircle>
          </SecondCircle>

          <ThirdCircle>
              <StyledCirculo src={circulo}/>
              <NumberCircle>{date?.numeros[2]}</NumberCircle>
          </ThirdCircle>

          <FourthCircle>
              <StyledCirculo src={circulo}/>
              <NumberCircle>{date?.numeros[3]}</NumberCircle>
          </FourthCircle>

          <FifthCircle>
              <StyledCirculo src={circulo}/>
              <NumberCircle>{date?.numeros[4]}</NumberCircle>
          </FifthCircle>

          <SixthCircle>
             <StyledCirculo src={circulo}/>
             <NumberCircle>{date?.numeros[5]}</NumberCircle>
          </SixthCircle>

          <SeventhCircle>
             <StyledCirculo src={circulo}/>
             <NumberCircle>{date?.numeros[6]}</NumberCircle>
          </SeventhCircle>

          </>

          : 

          <Loading>
            <CircularProgress />
          </Loading>
        
        }

           
        </ContainerMain>
    )
}
